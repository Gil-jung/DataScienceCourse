    ...
    try:
        with connection.cursor() as cursor:
            sql = ("SELECT a.s_id, a.s_name, a.school_score, a.csat_score "
                   ...
                   "	SELECT * "
                   "    FROM ( "
                   "    SELECT *, case when @prev_value1 = csat_score + (school_score * weight) and @prev_value2 = school_score then @vRank  else @vRank := @vRank + 1 end as rank, @vCounter := @vCounter + 1 as counter, @prev_value1 := csat_score + (school_score * weight), @prev_value2 := school_score "
                   "	FROM university natural join apply "
                   "					natural join student, "
                   "		(SELECT @vRank := 0, @vCounter := 0, @prev_value1 := NULL, @prev_value2 := NULL) r "
                   "	WHERE "
                   "		apply.u_id = %s and "
